...
\]
\textbf{Proof.} Let $\varepsilon>0$. Choose $\delta_1,\delta_2>0$ so that
$|f(x)-L|<\varepsilon/2$ and $|g(x)-M|<\varepsilon/2$. For
...
\]
and choosing the two errors sufficiently small makes the right side less than
any prescribed $\varepsilon$.
...
\]
\textbf{Proof.} Since $g(x)\to M\ne0$, eventually $|g(x)|>|M|/2$. Hence
\[
\left|\frac1{g(x)}-\frac1M\right|
=\frac{|g(x)-M|}{|g(x)||M|}
\le \frac{2|g(x)-M|}{|M|^2}\to0.
\]
Thus $1/g(x)\to1/M$, and the quotient rule follows from the product rule.
...
0<\sin\theta<\theta.
\]
\textbf{Proof.} By the mean value theorem applied to $\sin$ between $0$ and
$\theta$, $|\sin\theta|\le|\theta|$. Also
$1-\cos\theta=2\sin^2(\theta/2)\le |\theta|$ by the first inequality.
The final inequality is the standard unit-circle comparison for
$0<\theta\le\pi/2$.
\begin{center}
...
\section*{Additional examples from the lectures}

\subsection*{Uniqueness of the limit}
\textbf{Proposition.} A function cannot have two different limits at the same point.
If $\lim_{x\to c}f(x)=L$ and $\lim_{x\to c}f(x)=K$, then $L=K$.

\textbf{Proof.} Suppose $L\ne K$ and choose $\varepsilon=|L-K|/3$. The two
$\varepsilon$-neighborhoods of $L$ and $K$ are disjoint, but the definition of
limit would force $f(x)$ to lie in both for $x$ sufficiently close to $c$.
This contradiction proves $L=K$.

\subsection*{A useful $\varepsilon$-$\delta$ example}
...
\subsection*{Power and root rules}
...
\subsection*{One-sided limit example}
For $f(x)=|x|/x$,
\[
\lim_{x\to0^-}f(x)=-1,\qquad \lim_{x\to0^+}f(x)=1,
\]
so the two-sided limit does not exist. This is the simplest model of a jump
discontinuity.

\subsection*{Continuity test and types of discontinuity}
...
discontinuity because the one-sided limits are $0$ and $1$. At $1$, the
two-sided limit is $1$ but $f(1)=2$, so the discontinuity is removable: changing
the value at $1$ to $1$ would make the function continuous there.

\subsection*{Riemann-type example}
Define $R(x)=1/q$ when $x=p/q\in\mathbb Q\cap(0,1]$ is written in lowest terms,
and $R(x)=0$ when $x$ is irrational. At every irrational point, $R(x)\to0$;
at a rational point $p/q$, nearby rationals with large denominators make the
function values arbitrarily small, but the value at the point is positive.
Thus $R$ is continuous at irrational points and discontinuous at rational points.
